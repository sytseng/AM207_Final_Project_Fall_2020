...
class LUNA2:
    def __init__(self, architecture, random=None, weights=None):
        ...
        if weights is None:
            self.weights = self.random.normal(0, 1, size=(self.D, 1))
            self.weights_full = self.random.normal(0, 1, size=(self.D_full, 1))
            self.weights_aux = self.weights_full[self.D_theta:]
            self.theta = self.weights_full[:self.D_theta]
        else:
            self.weights = weights
            self.weights_full = weights

        ## making various LUNA data objects
        self.one_hot_mat = np.eye(self.params['D_in'])

        self.objective_trace = np.empty((1, 1))
        self.weight_trace = np.empty((1, self.D_full))

    def FiniteDiff(self, f_val, W, x_in, del_x, k):
        return (f_val - self.forward(W, del_x*self.one_hot_mat[k] + x_in)[0] )/del_x

    def make_LUNA_objects(self, W_aux, theta, x_train=None):
        self.W_mat = []
        self.f_fwd = []
        self.f_fwd_grad = []
        del_x = 0.001
        for i in range(self.params['M']):
            self.W_mat.append( np.array([np.append(theta, W_aux[ ( i*self.D_w):((i + 1)*self.D_w ) ] )]) )
            if x_train is not None:
                self.f_fwd.append(self.forward(self.W_mat[i], x_train) )
                grad_it = np.array([])
                for j, x_it in enumerate(x_train.T):
                    if np.shape(x_it)[0] == 1:
                        grad_it = np.append(grad_it, self.FiniteDiff(self.f_fwd[i][0][j], self.W_mat[i], np.array([x_it]), del_x, 0))

                    else: ## save this for later
                        for k in range(np.shape(x_it)):
                            np.append(grad_it, self.FiniteDiff(self.f_fwd[i][0][j], self.W_mat[i], np.array([x_it]), del_x, k))
                self.f_fwd_grad.append(np.array([grad_it]) )

    def forward_theta(self, theta, x_in):
        ...
        assert theta.shape[0] == self.D_theta
        assert x.shape[0] == D_in

        input = self.h(np.matmul( (theta[:D_in*H[0]]).reshape((H[0], D_in)), x) + theta[(D_in*H[0]):(D_in*H[0] + H[0])])
        ...
        for i in range(self.params['L'] - 1):
            ...
            input = self.h(np.matmul( (theta[index:(H[i]*H[i+1]+index) ]).reshape((H[i+1], H[i])), input) + theta[(H[i]*H[i+1]+index):(H[i]*H[i+1]+index+H[i+1])] )
            index += H[i+1]*H[i] + H[i+1]

        return input

    def forward_aux(self, theta_in, W_in):
        D_prev = self.params['H'][-1]
        D_out = self.params['D_out']
        D_w = D_prev*D_out



        assert theta_in.shape[0] == D_prev
        assert W_in.shape[1] == D_out

        return (np.matmul(W_in[:D_w].reshape((D_out, D_prev)), theta_in) + W_in[D_w:]).T
    ...
    def make_objective(self, x_train, y_train, lambda_in, reg_param=None):
        ...
        if reg_param is None:
            def objective(W_full, t):
                W_mat = self.build_weights_mat(W_full)
                sum_error = 0
                sim_it = 0
                for i, W in enumerate(W_mat):
    ...
    def _CoSim(self, grad1, grad2):
        return ((np.dot(grad1.flatten(), grad2.flatten()) )**2)/((np.sum(np.square(grad1.flatten()) ) )*(np.sum(np.square(grad2.flatten()) )  ))

    def make_objective2(self, x_train, y_train, lambda_in, reg_param=None):
        D_prev = self.params['H'][-1]
        D_out = self.params['D_out']
        D_w = D_prev*D_out
        del_x = 0.0001
        ...
        if reg_param == None:
            def objective(W_full, t):
                ...
                weights_tens = W_full[self.D_theta:self.D_theta +D_out*D_prev*self.params['M']].reshape((self.params['M'], D_out, D_prev))
                b_tens = W_full[self.D_theta + D_out*D_prev*self.params['M']:].reshape((self.params['M'], D_out, 1))
                fwd_list = np.array([self.forward_theta(W_full[:self.D_theta], x_train)])
                grad_list = np.array(np.matmul(weights_tens, fwd_list[0]) + b_tens)
                for i, vec in enumerate(self.one_hot_mat):
                    fwd_list = np.vstack((fwd_list, [self.forward_theta(W_full[:self.D_theta], np.add(x_train, del_x*vec))] ))
                    grad_list = np.hstack((grad_list, (np.matmul(weights_tens, fwd_list[i+1]) + b_tens - grad_list[0])/del_x))

                grad_dot = np.array([])
                for i, gradi in enumerate(grad_list):
                    if i == 0:
                        error_it += np.linalg.norm(gradi[0].T - y_train)
                        gradi_sqr = np.sum(np.matmul(gradi[1:].T, gradi[1:] ))
                        grad_dot = np.append(grad_dot, gradi_sqr)
                    for j, gradj in enumerate(grad_list[i+1:]):
                        if i == 0:
                            error_it += np.linalg.norm(gradj[0].T - y_train)
                            gradj_sqr = np.sum(np.matmul(gradj[1:].T, gradj[1:] ))
                            grad_dot = np.append(grad_dot, gradj_sqr)
                        sim_it += (np.sum(np.matmul(gradi[1:].T, gradj[1:] ))**2)/grad_dot[i]/grad_dot[j]

                mean_error = error_it + lambda_in*sim_it
                return mean_error
        else:
            def objective(W_full, t):
                error_it = 0.0
                ...
                for i, gradi in enumerate(grad_list):
                    ...
                    for j, gradj in enumerate(grad_list[i+1:]):
                        print("gradj", t, i, j)
                        gradj_sqr = np.sum(np.matmul(gradj[1:].T, gradj[1:] ))
                        sim_it += (np.sum(np.matmul(gradi[1:].T, gradj[1:] ))**2)/gradi_sqr/gradi_sqr
    ...
    def fit(self, x_train, y_train, params, lambda_in, reg_param=None):
        ...
        assert x_train.shape[1] == self.params['D_in']
        assert y_train.shape[1] == self.params['D_out']

        ### make objective function for training
        self.objective, self.gradient = self.make_objective2(x_train, y_train, lambda_in, reg_param)

        ### set up optimization
        step_size = 0.01
        max_iteration = 5000
        check_point = 100
        weights_init = self.weights_full
